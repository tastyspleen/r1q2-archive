...
//int (WINAPI *pDirectSoundCreate8)(GUID FAR *lpGUID, LPDIRECTSOUND8 FAR *lplpDS, IUnknown FAR *pUnkOuter);
...
sndinitstat SNDDMA_InitDirect (void)
{
	...
	Com_Printf ("Speaker config: ");
	pDS->lpVtbl->GetSpeakerConfig (pDS, &speakerconfig);
	...
	switch (speakerconfig)
	{
		...
		case DSSPEAKER_DIRECTOUT:
			Com_Printf ("DIRECTOUT.\n");
			break;
		case DSSPEAKER_HEADPHONE:
			Com_Printf ("HEADPHONE.\n");
			break;
		case DSSPEAKER_MONO:
			Com_Printf ("MONO.\n");
			break;
...
// to have sound)
	...
	{
		Con_Print ("Initializing waveOut. Use snd_restart 1 to retry full sound init.\n");
