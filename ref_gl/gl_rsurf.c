...
{
	if ( FLOAT_EQ_ZERO(soffset) && FLOAT_EQ_ZERO(toffset))
	{
		for ( ; p != 0; p = p->chain )
		{
			float *v;
	...
	*/
	if ( FLOAT_NE_ZERO(gl_dynamic->value) )
	{
				...
				for ( drawsurf = newdrawsurf; drawsurf != surf; drawsurf = drawsurf->lightmapchain )
				{
					if ( drawsurf->polys )
						DrawGLPolyChain( drawsurf->polys, 
							              ( drawsurf->light_s - drawsurf->dlight_s ) * 0.0078125f, 
										( drawsurf->light_t - drawsurf->dlight_t ) * 0.0078125f );
				}
		...
		*/
		...
		{
			if ( surf->polys )
				DrawGLPolyChain( surf->polys, ( surf->light_s - surf->dlight_s ) * ( 0.0078125f ), ( surf->light_t - surf->dlight_t ) * ( 0.0078125f ) );
		}
	}
...
void R_RenderBrushPoly (msurface_t *fa)
{
	...
	else
	{
		...
		/*if (image->detailtexnum) {
			...
			qglLoadIdentity();
			qglScalef((float)1.0, (float)1.0, 1);
...
void R_DrawBrushModel (entity_t *e)
{
	...
	currententity = e;
	gl_state.currenttextures[0] = gl_state.currenttextures[1] = 0xFFFFFFFF;
