...
void SubdividePolygon (int numverts, float *verts)
{
	...
	if (numverts > 60)
		ri.Sys_Error (ERR_DROP, "SubdividePolygon: numverts = %i", numverts);
...
void MakeSkyVec (float s, float t, int axis)
{
	...
	b[0] = s * (gl_zfar->value / 2);
	b[1] = t * (gl_zfar->value / 2);
	b[2] = (gl_zfar->value / 2);
