...
void CL_SmokeTrail (vec3_t start, vec3_t end, int colorStart, int colorRun, int spacing)
{
	...
	while (len > 0)
	{
		...
		p->color = colorStart + (float)(randomMT() % colorRun);
		for (j=0 ; j<3 ; j++)
		{
...
//			c = cos(rot)/2.0;
//			s = sin(rot)/2.0;
//			variance = 0.4 + ((float)randomMT()/(float)RAND_MAX) *0.2;
			variance = 0.5;
			c = cos(rot)*variance;
			s = sin(rot)*variance;
...
void CL_Nukeblast (cl_sustain_t *self)
{
	...
	for(i=0;i<700;i++)
	{
		...
		VectorMA(self->org, (200.0 * ratio * crand()), dir, p->org);
//		VectorMA(origin, 10*(((rand () & 0x7fff) / ((float)0x7fff))), dir, p->org);
	}
}
...
void CL_ColorExplosionParticles (vec3_t org, int color, int run)
{
	...
	for (i=0 ; i<128 ; i++)
	{
		...
		p->time = cl.time;
		p->color = color + (float)(randomMT() % run);
