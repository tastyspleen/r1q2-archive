...
void CL_ParseMuzzleFlash2 (void) 
{
	...
	case MZ2_TANK_MACHINEGUN_19:
		...
		CL_ParticleEffect (origin, vec3_origin, 0, 40);
		CL_SmokeAndFlash(origin);
		Com_sprintf(soundname, sizeof(soundname), "tank/tnkatk2%c.wav", 'a' + randomMT() % 5);
...
void CL_ParticleEffect (vec3_t org, vec3_t dir, int color, int count)
{
	...
	for (i=0 ; i<count ; i++)
	{
		...
		p->time = cl.time;
		p->color = color + (float)(randomMT()&7);
...
void CL_BigTeleportParticles (vec3_t org)
{
	...
	for (i=0 ; i<4096 ; i++)
	{
		...
		angle = M_PI*2*(randomMT()&1023)/1023.0;
		dist = randomMT()&31;
		p->org[0] = org[0] + cos(angle)*dist;
		p->vel[0] = cos(angle)*(70+(randomMT()&63));
		p->accel[0] = -cos(angle)*100;

		p->org[1] = org[1] + sin(angle)*dist;
		p->vel[1] = sin(angle)*(70+(randomMT()&63));
...
void CL_FlyParticles (vec3_t origin, int count)
{
	...
	if (!avelocities[0][0])
	{
		for (i=0 ; i<NUMVERTEXNORMALS*3 ; i++)
			avelocities[0][i] = (randomMT()&255) * 0.01;
	}
	...
	for (i=-2 ; i<=2 ; i+=4)
		for (j=-2 ; j<=2 ; j+=4)
			for (k=-2 ; k<=4 ; k+=4)
			{
				...
				p->alpha = 1.0;
				p->alphavel = -1.0 / (0.3 + (randomMT()&7) * 0.02);
